 ...
 // ItemBuilder _itemBuilder;
    ...
    return FutureBuilder(
      ...
      builder: (context, snapshot) {
        ...
        /*if (map.containsKey("Top Punjabi Albums")) {
          widgets.add(_itemBuilder
              .buildTopAlbums(List<Details>.from(map["Top Punjabi Albums"])));
        }
        if (map.containsKey("Top 20 Songs")) {
          List<Song> songs =
          List<Song>.from(map["Top 20 Songs"]).take(4).toList();
          widgets.add(
              _itemBuilder.buildTrendingPlaylist("Editorial Picks", songs));
        }
          ...
          return CupertinoActionSheet(
            actions: <Widget>[
              CupertinoActionSheetAction(
                onPressed: () {
                  //SoundWaveState.launchURL(
                  //    "https://github.com/singhbhavneet/Bungee/issues");
                },
                child: Text(
                  "Report...",
              ...
              CupertinoActionSheetAction(
                onPressed: () {
                 // SoundWaveState.share(
                   //   "https://play.google.com/store/apps/details?id=com.blackhole.soundwave");
                },
                child: Text(
                  "Share",
                  style: Theme.of(context).textTheme.title.copyWith(
  ...
  Widget createChoices() {
    ...
    choices.add(Choice(
        ...
        "library/punjabi albums"));
    choices.add(Choice("Surprise", null, "library/bollywood mix"));
    return ListView.builder(
      itemBuilder: (context, index) => choices[index].buildWidget(context),
